...
브랜치를 만들면 `main`의 안정된 코드를 건드리지 않고 새 작업을 진행할 수 있음. 작업이 완료되고 검증된 후에 `main`에 병합하므로, 개발 중인 코드가 배포 코드에 영향을 주지 않음. 팀 협업에서는 각자 브랜치에서 작업한 뒤 PR(Pull Request)로 검토 후 병합하는 방식으로 확장됨.
