...
이번 과제를 통해 Python을 처음 배우면서, 배운 내용을 직접 퀴즈로 만들면 개념을 한 번 더 정리할 수 있다고 생각했습니다.
...
`feature/gitignore` 브랜치를 생성해 `.gitignore` 업데이트 작업을 진행한 뒤 `main`으로 병합했습니다.
...
- **Quiz**: 퀴즈 한 문제를 표현하는 단위 객체입니다. 문제 출력(`display`)과 정답 확인(`check_answer`) 같이 퀴즈 자체에 관한 동작만 담당합니다.
- **QuizGame**: 게임 전체 흐름을 관리합니다. 퀴즈 목록 유지, 메뉴 출력, 기능 실행, 파일 저장/불러오기를 담당합니다.

이렇게 나누면 퀴즈 한 문제의 로직이 바뀌어도 `Quiz`만 수정하면 되고, 게임 흐름이 바뀌면 `QuizGame`만 수정하면 됩니다.
...
`KeyboardInterrupt`(Ctrl+C)와 `EOFError`(입력 스트림 종료) 두 경우를 최상위에서 잡아 안내 메시지를 출력하고 종료합니다. 데이터는 변경 시점마다 즉시 저장하므로 이 시점에 별도 저장이 필요 없습니다.

### 커밋 단위 및 메시지 규칙

기능이 완성될 때마다 커밋했습니다. 메시지 앞에 작업 유형을 붙여 히스토리에서 변경 의도를 바로 알 수 있게 했습니다.
...
함수만 사용하면 퀴즈 데이터(문제, 선택지, 정답)를 여러 변수로 따로 관리해야 합니다. 클래스를 쓰면 관련 데이터와 동작을 하나로 묶을 수 있어 코드가 명확해집니다.

예를 들어 `quiz.check_answer(3)`처럼 퀴즈 객체 스스로 정답을 판단하게 만들 수 있습니다. 반면 함수 방식이면 `check_answer(question, choices, answer, 3)` 식으로 데이터를 계속 전달해야 합니다.

### JSON 파일 저장 이유

- 텍스트 형식이라 사람이 직접 읽고 수정할 수 있습니다.
- Python 표준 라이브러리 `json`으로 바로 읽고 쓸 수 있습니다.
- 딕셔너리·리스트 구조를 그대로 저장할 수 있어 퀴즈 데이터 형태와 잘 맞습니다.

### 파일 입출력에서 try/except가 필요한 이유

파일 작업은 프로그램 외부 환경(디스크, OS)에 의존하므로 예측 불가능한 오류가 발생할 수 있습니다.

- 파일이 다른 프로그램에 의해 손상된 경우 → `json.JSONDecodeError`
- 디스크 읽기 실패 → `IOError`

이런 오류를 처리하지 않으면 프로그램이 즉시 종료됩니다. `try/except`로 감싸면 오류가 발생해도 기본 데이터로 복구하고 실행을 계속할 수 있습니다.

### 브랜치 분리 이유 및 병합의 의미

브랜치를 만들면 `main`의 안정된 코드를 건드리지 않고 새 작업을 진행할 수 있습니다. 작업이 완료되고 검증된 후에 `main`에 병합하므로, 개발 중인 코드가 배포 코드에 영향을 주지 않습니다. 팀 협업에서는 각자 브랜치에서 작업한 뒤 PR(Pull Request)로 검토 후 병합하는 방식으로 확장됩니다.
...
최상위를 딕셔너리로 만들어 키로 데이터 종류를 구분했습니다. 나중에 `score_history` 같은 항목을 추가해도 기존 키에 영향을 주지 않습니다. `quizzes`는 리스트로, 각 퀴즈는 딕셔너리로 저장해 `Quiz` 객체와 1:1로 대응되도록 설계했습니다.

### 퀴즈 데이터 증가 시 JSON 방식의 한계

- 퀴즈가 수천 개가 되면 매번 전체 파일을 읽고 쓰므로 속도가 느려집니다.
- 동시에 두 프로세스가 저장하면 데이터가 겹쳐 쓰일 수 있습니다(동시성 문제).
- 검색이나 정렬 기능이 필요해지면 코드로 직접 구현해야 합니다.

이런 상황이 되면 SQLite 같은 경량 데이터베이스로 전환하는 것이 적합합니다.

### state.json 손상 시 대응 방안

현재 구현에서는 `load()` 안의 `try/except`가 손상된 파일을 감지하면 기본 퀴즈 데이터로 초기화합니다.
...
추가 대응 방안으로는 저장 전 백업 파일(`state.json.bak`)을 만들어두면 손상 시 복구가 가능합니다.
